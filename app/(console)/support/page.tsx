...
export default function SupportPage() {
  ...
  const refreshPreview = useCallback(async (note = userNote) => {
    try {
      setPreview(await api.supportPreview(note));
    } catch {
      // Preview is optional; keep the last snapshot when refresh fails.
    }
  }, [userNote]);
  ...
  const loadTicket = async (ticketId: string) => {
    try {
      setTicket(await api.supportTicket(ticketId));
    } catch {
      // History item may have expired; ignore stale selections.
    }
  };
  ...
  const load = useCallback(async () => {
    setError(null);
    try {
      const entitlementRes = await api.supportEntitlement();
  ...
  const submit = async () => {
    ...
    setTicket(null);
    try {
      const { ticket_id } = await api.supportSubmit(userNote);
      setSubmitting(false);
      await pollTicket(ticket_id);
    } catch (e) {
      setSubmitError(e instanceof ApiError ? e.message : e instanceof Error ? e.message : 'Failed to send report');
      setSubmitting(false);
    }
  };
